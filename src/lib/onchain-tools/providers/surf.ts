...
export async function getSurfSmartMoneyResearch(
  ...
  const query = (options.query || "Find smart-money accumulation on Mantle").trim();
...
export function getSurfSmartMoneyCoverage(
  ...
  const query = (
    options.query ||
    `Find smart-money accumulation on ${options.chain || "Mantle"}`
  ).trim();
  const target = resolveSmartMoneyTarget(options, query);
...
function buildSmartMoneyUserPrompt(
  ...
  const chain = options.chain || "mantle";
...
function normalizeChainId(value: string | undefined) {
  const normalized = value?.trim().toLowerCase() || "mantle";
