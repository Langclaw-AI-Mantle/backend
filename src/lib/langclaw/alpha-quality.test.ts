...
const smartMoneyReport: ResearchReport = {
  ...
  confidence: "high",
  entities: [],
  executiveSummary: "Direct smart-money evidence exists on Mantle.",
  kind: "smart-money",
...
test("high quality Mantle smart-money signal becomes alert eligible", () => {
  const alphaSignal = buildAlphaSignal({
    ...
    sources: [
      {
        excerpt: "Mantle smart-money flow context.",
        id: "source-1",
        provider: "Surf",
        title: "Mantle context",
        type: "docs_page",
        url: "https://example.com",
      },
    ],
    topic: "Find smart-money accumulation on Mantle",
  });
...
test("external low-confidence fallback cannot trigger alpha alert", () => {
  const alphaSignal = buildAlphaSignal({
    ...
    signals,
    sources: [],
    topic: "Find smart-money accumulation for MNT on Mantle",
  });
...
test("supplemental external context warns but does not block direct Mantle alpha", () => {
  const alphaSignal = buildAlphaSignal({
    chainContext,
    onChain: {
      ...onChain,
      tools: onChain.tools.map((tool) => ({
        ...
        summary:
          "Supplemental Ethereum MNT CEX transfer lookup returned 1 row as external low-confidence context.",
      })),
    },
    proof,
    report: {
...
test("negated external fallback text in user prompt does not trip the external guard", () => {
  const alphaSignal = buildAlphaSignal({
    ...
    sources: [],
    topic:
      "Find smart-money accumulation on Mantle. Do not use external low-confidence token context.",
  });
