...
export function buildAlphaSignalQuality({
  ...
  const alertEligible =
    ...
    chainContext.productChain.id === "mantle" &&
...
function buildFalsePositiveChecks({
  ...
  checks.push({
    ...
    reason:
      chainContext.productChain.id === "mantle"
        ? "The decision is scoped to Mantle."
        : `The product chain is ${chainContext.productChain.name}, not Mantle.`,
    status: chainContext.productChain.id === "mantle" ? "pass" : "fail",
  });
  checks.push({
    id: "onchain_core_source",
    label: "Mantle on-chain data",
  ...
  if (onChain?.plan.analysisSource === "prompt" && onChain.plan.chain !== "mantle") {
    checks.push({
      id: "analysis_chain_scope",
      label: "Analysis chain scope",
      reason: `The analysis chain is ${onChain.plan.chainName}, so it should not be submitted as Mantle alpha.`,
      status: "fail",
    });
  }
...
function readExternalFallbackStatus({
  ...
  if (selectedExternalFallback) {
    return {
      reason:
        "External token fallback became the selected smart-money route, so it cannot trigger a Mantle alpha alert.",
      status: "fail",
    };
  }
  ...
  return {
    reason: "No external low-confidence fallback was promoted as Mantle alpha.",
    status: "pass",
  };
}
